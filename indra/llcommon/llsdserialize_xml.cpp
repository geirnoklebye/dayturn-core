								  ...
								  EFormatterOptions options, U32 level) const
{
	...
	case LLSD::TypeBinary:
	{
		...
		else
		{
            ...
            auto b64_buffer_length = apr_base64_encode_len(narrow(buffer.size()));
...
void LLSDXMLParser::Impl::parsePart(const char* buf, std::streamsize len)
{
	if ( buf != nullptr
		&& len > 0 )
	{
...
void LLSDXMLParser::Impl::endElementHandler(const XML_Char* name)
{
	...
	switch (element)
	{
		...
		case ELEMENT_BINARY:
		{
			...
			unsigned int len = apr_base64_decode_len(stripped.c_str());
			std::vector<U8> data;
			data.resize(len);
...
void LLSDXMLParser::parsePart(const char *buf, std::streamsize len)
{
