...
public:
	 ...
	 */
	S32 parse(std::istream& istr, LLSD& data, std::streamsize max_bytes, S32 max_depth = -1);
...
protected:
	 ...
	 */
	void account(std::streamsize bytes) const;
...
private:
	...
	void parsePart(const char* buf, std::streamsize len);
...
public:
	 ...
	 */
	static bool deserialize(LLSD& sd, std::istream& str, std::streamsize max_bytes);
	...
	static S32 fromNotation(LLSD& sd, std::istream& str, std::streamsize max_bytes)
	{
		LLPointer<LLSDNotationParser> p = new LLSDNotationParser;
		return p->parse(str, sd, max_bytes);
	}
	static LLSD fromNotation(std::istream& str, std::streamsize max_bytes)
	{
		LLPointer<LLSDNotationParser> p = new LLSDNotationParser;
		LLSD sd;
...
//		return fromXMLDocument(sd, str, emit_errors);
	...
	static S32 fromBinary(LLSD& sd, std::istream& str, std::streamsize max_bytes, S32 max_depth = -1)
	{
		LLPointer<LLSDBinaryParser> p = new LLSDBinaryParser;
		return p->parse(str, sd, max_bytes, max_depth);
	}
	static LLSD fromBinary(std::istream& str, std::streamsize max_bytes, S32 max_depth = -1)
	{
		LLPointer<LLSDBinaryParser> p = new LLSDBinaryParser;
		LLSD sd;
...
public:
    ...
    static EZipRresult unzip_llsd(LLSD& data, std::istream& is, uint32_t size);
    static EZipRresult unzip_llsd(LLSD& data, const U8* in, uint32_t size);
...
LL_COMMON_API char* strip_deprecated_header(char* in, std::streamsize& cur_size, size_t* header_size = nullptr);
