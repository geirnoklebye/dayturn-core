...
namespace FSCoreHttpUtil
{
	...
	void callbackHttpPostRaw(const std::string &url, std::string postData, completionCallback_t success = 0, completionCallback_t failure = 0,
							 LLCore::HttpHeaders::ptr_t aHeader = LLCore::HttpHeaders::ptr_t(), LLCore::HttpOptions::ptr_t options = LLCore::HttpOptions::ptr_t());
	void callbackHttpGetRaw(const std::string &url, completionCallback_t success = 0, completionCallback_t failure = 0,
							LLCore::HttpHeaders::ptr_t aHeader = LLCore::HttpHeaders::ptr_t(), LLCore::HttpOptions::ptr_t options = LLCore::HttpOptions::ptr_t());
