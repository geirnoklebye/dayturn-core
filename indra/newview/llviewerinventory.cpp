...
// CA: This RLVa code is used fulltime, so abstract it to where it's needed

struct localPredIsEqualOrLinkedItem
{
	localPredIsEqualOrLinkedItem(const LLViewerInventoryItem* pItem) : m_pItem(pItem) {}
	localPredIsEqualOrLinkedItem(const LLUUID& idItem);
...
localPredIsEqualOrLinkedItem::localPredIsEqualOrLinkedItem(const LLUUID& idItem)
{
	m_pItem = gInventory.getItem(idItem);
}

// Checked: 2010-04-24 (RLVa-1.2.0f) | Added: RLVa-1.2.0f
bool localPredIsEqualOrLinkedItem::operator()(const LLViewerInventoryItem* pItem) const
{
...
void sync_inventory_folder(const LLUUID& folder_id, const LLInventoryModel::item_array_t& items, LLInventoryModel::item_array_t& items_to_add, LLInventoryModel::item_array_t& items_to_remove)
{
	...
	for (LLInventoryModel::item_array_t::const_iterator itCurItem = curItems.begin(); itCurItem != curItems.end(); ++itCurItem)
	{
		...
		if (std::find_if(newItems.begin(), newItems.end(), localPredIsEqualOrLinkedItem(pItem)) == newItems.end())
		{
		...
		else
		{
			...
			newItems.erase(std::remove_if(newItems.begin(), newItems.end(), localPredIsEqualOrLinkedItem(pItem)), newItems.end());
		}
	}
