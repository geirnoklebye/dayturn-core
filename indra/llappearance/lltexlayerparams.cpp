...
LLColor4 LLTexLayerParamColor::getNetColor() const
{
	...
	else
	{
		...
		return LLColor4((1.f - weight) * start->mV[VX] + weight * end->mV[VRED],
						(1.f - weight) * start->mV[VY] + weight * end->mV[VGREEN],
						(1.f - weight) * start->mV[VZ] + weight * end->mV[VBLUE],
						(1.f - weight) * start->mV[VW] + weight * end->mV[VALPHA]);
	}
}
